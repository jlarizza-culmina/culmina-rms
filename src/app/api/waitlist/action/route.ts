...
export async function POST(request: NextRequest) {
  try {
    ...
    // action: 'notify' | 'seat' | 'no_show' | 'cancel'
    ...
    if (action === 'notify') {
      await supabaseAdmin.from('waitlist_sessions')
        .update({ status: 'notified', notified_at: now })
        .eq('id', sessionId)

      await sendSMS(
        session.phone,
        `${session.guest_name}, your table is ready at ${restaurantName}! Head over now. 🍷`
      )
    } else if (action === 'seat') {
      await supabaseAdmin.from('waitlist_sessions')
        .update({ status: 'seated', seated_at: now })
        .eq('id', sessionId)
      ...
      await sendSMS(
        session.phone,
        `Your waitlist spot at ${restaurantName} has been cancelled. Hope to see you soon!`
      )
    }
