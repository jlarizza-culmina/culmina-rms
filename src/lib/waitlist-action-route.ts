...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'notify') {
      ...
      // Update guest last_visit_date on seat confirmation
      if (session.guest_id) {
        await supabase.from('guests').update({ last_visit_date: new Date().toISOString().split('T')[0] }).eq('id', session.guest_id)
      }
