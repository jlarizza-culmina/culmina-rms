...
export default function RecipeView({
  ...
  onTabChange, onServingsChange, onToggleCheck, onClearChecks, onDelete, onCookMode, onUpdateRecipe
}: Props) {
      ...
      <div className="bg-white border-b border-[--border] px-6 pt-5 pb-0">
        <div className="flex items-start justify-between mb-1.5">
          ...
          <div className="flex gap-2 flex-shrink-0">
            <button
              onClick={onCookMode}
              title={isCocktail ? 'Build guide' : 'Cook mode'}
            ...
            <button onClick={onDelete} className="text-[--hint] hover:text-red-500 text-sm px-2 transition-colors" title="Delete">✕</button>
          </div>
        </div>
        {recipe.description && (
