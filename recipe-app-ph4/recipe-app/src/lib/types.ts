...
export type AppMode = 'cookbook' | 'library' | 'report' | 'production' | 'schedule'
...
export interface ShoppingListItem {
  key: string                // name::unit dedup key
  name: string
  category: string
  is_bar: boolean
  ...
  purchase_amount?: number   // adjusted for trim + waste buffer
  unit_cost?: number | null
  total_cost?: number
  recipe_names: string[]     // which recipes need this
}
...
export interface BatchSuggestion {
  ...
  waste: number
  multiplier: number         // total_portions / base_servings
}
...
// Internal to TMinusSchedule — computed from recipe steps
export interface PrepTask {
  id: string                  // recipeId-stepId
  recipeId: string
  recipeName: string
  stepId: string
  stepTitle: string
  ...
  displayEnd: string          // "8:30 AM"
  tMinusLabel: string         // "T−6h 30m"
  isSubRecipe: boolean
  parentRecipeName?: string
  color: string               // recipe color for visual grouping
}
