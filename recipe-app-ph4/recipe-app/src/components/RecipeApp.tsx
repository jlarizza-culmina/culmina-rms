...
export default function RecipeApp({ user }: Props) {
  ...
  // ── Nav button helper — icon only, tooltip on hover ──────────
  const NavBtn = ({ mode, icon, title }: { mode: AppMode; icon: string; title: string }) => (
    <button
      onClick={() => setAppMode(mode)}
      title={title}
      className={`flex-1 py-2 rounded-md text-base transition-all flex items-center justify-center
        ${appMode === mode ? 'bg-white shadow-sm' : 'hover:bg-white/60'}`}>
      {icon}
    </button>
  )
    ...
    <div className="flex h-screen overflow-hidden bg-[--bg]">
      ...
      <aside className="w-56 min-w-56 bg-white border-r border-[--border] flex flex-col">
        <div className="px-3 py-3 border-b border-[--border] space-y-2">
          ...
          {/* App mode nav — icon only, 5 modes */}
          <div className="flex bg-[--surface-2] rounded-lg p-0.5 gap-0.5">
            <NavBtn mode="cookbook"    icon="📖" title="Cookbook" />
            <NavBtn mode="library"     icon="📦" title="Ingredient Library" />
            <NavBtn mode="production"  icon="📋" title="Production Planner" />
            <NavBtn mode="schedule"    icon="⏰" title="T-Minus Schedule" />
            <NavBtn mode="report"      icon="📊" title="Cost Report" />
          </div>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        {appMode === 'schedule' && (
          <TMinusSchedule recipes={recipes} userId={user.id} />
        )}
